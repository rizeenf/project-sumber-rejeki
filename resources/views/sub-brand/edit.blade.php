    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <div class="card mb-4">
          ...
          <!-- Form -->
          <div class="card-body">
            <form method="POST" action="{{ route('sub-brand.update', $subBrand->id) }}">
                @csrf
                @method('PUT')
              <div class="row">
                <div class="mb-3 col-md-12">
                    <label for="name" class="form-label">Nama</label>
                    <input class="form-control" type="text" id="name" name="name" value="{{ $subBrand->name }}"/>
                </div>
                ...
                <div class="mb-3 col-md-12">
                  ...
                  <select id="category" class="select2 form-select" name="category">
                    ...
                    @foreach ($categories as $category)
                      <option value="{{ $category->id }}" {{ $subBrand->category_product_id == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                    @endforeach
                  </select>
                </div>
                ...
                <div class="mb-3 col-md-12">
                  ...
                  <select id="brand" class="select2 form-select" name="brand">
                    ...
                    @foreach ($brands as $brand)
                      <option value="{{ $brand->id }}" {{ $subBrand->brand_product_id == $brand->id ? 'selected' : '' }}>{{ $brand->name }}</option>
                    @endforeach
                  </select>
                </div>
              </div>
