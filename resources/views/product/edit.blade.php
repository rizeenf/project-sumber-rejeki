                ...
                <div class="mb-3 col-md-12">
                  ...
                  <div class="input-group">
                    <div class="input-group-text">
                      <input class="form-check-input mt-0" type="checkbox" value="1" name="competitor" {{ $product->competitor == 1 ? 'checked' : '' }}/>
                    </div>
                    <input type="text" class="form-control" name="competitor_name" placeholder="Nama Kompetitor" value="{{ $product->competitor_name }}"/>
                  </div>
                </div>
                ...
                <div class="mb-3 col-md-12">
                  ...
                  <select id="brand" class="select2 form-select" name="brand">
                    ...
                    @foreach ($brands as $brand)
                      <option value="{{ $brand->id }}" {{ $product->brand_product_id == $category->id ? 'selected' : '' }}>{{ $brand->name }}</option>
                    @endforeach
                  </select>
                </div>
                ...
                <div class="mb-3 col-md-12">
                  ...
                  <select id="sub_brand" class="select2 form-select" name="sub_brand">
                    ...
                    @foreach ($subBrands as $subBrand)
                      <option value="{{ $subBrand->id }}" {{ $product->sub_brand_product_id == $category->id ? 'selected' : '' }}>{{ $subBrand->name }}</option>
                    @endforeach
                  </select>
                </div>
